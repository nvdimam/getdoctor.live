import os
import sys
import getdoctor.config.settings

# create settings object corresponding to specified env
APP_ENV = os.environ.get('APP_ENV', 'Dev')
_current = getattr(sys.modules['getdoctor.config.settings'], '{0}Config'.format(APP_ENV))()
...
def as_dict():
   res = {}
   for atr in [f for f in dir(getdoctor.config) if not '__' in f]:
       val = getattr(getdoctor.config, atr)
       res[atr] = val
   return res